...
const StarIcon = ({ filled, glowing }) => (
  <svg
    className={`w-12 h-12 transition-all duration-150 ${
      filled
        ? `text-amber-400 ${glowing ? 'drop-shadow-[0_0_8px_rgba(251,191,36,0.6)]' : ''}`
        : 'text-gray-200'
    }`}
    fill="currentColor"
    viewBox="0 0 20 20"
...
export default function ReviewFlow() {
  ...
  useEffect(() => {
    api
      .get(`/reviews/campaign/${campaignId}/public`)
  ...
  const handleRating = async (val) => {
    ...
    try {
      const [aiRes, submitRes] = await Promise.all([
        api.post('/ai/generate', { businessName: campaign.businessName, rating: val }),
        api.post(`/reviews/${campaignId}`, { rating: val, answers: {} }),
      ]);

      setReviews(aiRes.data.reviews);
      setRedirectUrl(submitRes.data.redirect || '');
      setStep('select');
    } catch (err) {
  ...
  const handleSelectReview = async (text) => {
    try {
      ...
      // clipboard blocked in some browsers — proceed anyway
    }
      ...
      <div className="min-h-screen bg-gradient-to-b from-slate-50 to-gray-100 flex flex-col items-center justify-center px-4 py-12">
        <div className="w-full max-w-md">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-xl font-semibold text-gray-900">{campaign.businessName}</h1>
          ...
          {(step === 'rating' || step === 'generating') && (
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 px-8 py-10 text-center">
              ...
              {rating > 0 && !loading && (
                <p className={`mt-4 text-sm font-semibold tracking-wide ${RATING_COLORS[rating]}`}>
          ...
          {/* Step: Select a review */}
          {step === 'select' && (
            <div>
              <div className="text-center mb-6">
                <div className="flex justify-center gap-1 mb-3">
                  {[1, 2, 3, 4, 5].map((i) => (
                    <StarIcon key={i} filled={i <= rating} glowing={false} />
                  ))}
                </div>
          ...
          <p className="text-center text-xs text-gray-300 mt-8">Powered by ReviewBoost</p>
        </div>
      </div>
    </>
  );
