...
export default function CampaignDetail() {
  ...
  const downloadQR = () => {
    ...
    link.href = campaign.qrCodeDataUrl;
    link.download = `${campaign.name}-qr.png`;
    link.click();
  };

  const toggleActive = async () => {
    const updated = await api.patch(`/campaigns/${id}`, { active: !campaign.active });
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex items-start justify-between gap-4 mb-6">
          ...
          <div className="flex gap-2 flex-shrink-0">
            <button onClick={toggleActive} className="btn-secondary text-sm">
        ...
        {tab === 'overview' && (
          <div className="space-y-6">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                { label: 'Total reviews', value: campaign.stats.totalReviews, color: 'text-gray-900' },
                { label: 'Sent to Google', value: campaign.stats.positiveRedirects, color: 'text-green-600' },
                { label: 'Private feedback', value: campaign.stats.negativeFeedbacks, color: 'text-gray-900' },
                { label: 'Avg rating', value: campaign.avgRating || '—', color: 'text-brand-600' },
              ].map((s) => (
            ...
            <div className="card p-5">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={copyLink} className="btn-secondary text-sm flex-shrink-0">
                  {copied ? 'Copied!' : 'Copy'}
                </button>
                <a href={reviewUrl} target="_blank" rel="noreferrer" className="btn-secondary text-sm flex-shrink-0">
                  Open
                </a>
              </div>
            </div>
            ...
            {/* Conversion */}
            {campaign.stats.totalReviews > 0 && (
              <div className="card p-5">
                <h3 className="text-sm font-medium text-gray-700 mb-4">Conversion</h3>
                <div className="space-y-3">
                  <div>
                    <div className="flex justify-between text-sm mb-1">
                      <span className="text-gray-600">Sent to Google</span>
                      <span className="font-medium text-green-600">
                        {Math.round((campaign.stats.positiveRedirects / campaign.stats.totalReviews) * 100)}%
                      </span>
                    </div>
                    <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                      <div
                        className="h-full bg-green-500 rounded-full"
                        style={{ width: `${(campaign.stats.positiveRedirects / campaign.stats.totalReviews) * 100}%` }}
                      />
                    </div>
                  </div>
                  <div>
                    <div className="flex justify-between text-sm mb-1">
                      <span className="text-gray-600">Private feedback</span>
                      <span className="font-medium text-gray-500">
                        {Math.round((campaign.stats.negativeFeedbacks / campaign.stats.totalReviews) * 100)}%
                      </span>
                    </div>
                    <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                      <div
                        className="h-full bg-gray-400 rounded-full"
                        style={{ width: `${(campaign.stats.negativeFeedbacks / campaign.stats.totalReviews) * 100}%` }}
                      />
                    </div>
                  </div>
                </div>
        ...
        {tab === 'qr-code' && (
          <div className="card p-8 flex flex-col items-center text-center max-w-sm mx-auto">
            ...
            <p className="text-sm text-gray-500 mb-6">
              Print this on receipts, table cards, or displays.
            </p>
            {campaign.qrCodeDataUrl ? (
              <>
                <img
                  src={campaign.qrCodeDataUrl}
                  alt="QR Code"
                  className="w-56 h-56 rounded-xl border border-gray-100 shadow-sm"
                />
                <p className="text-xs text-gray-400 mt-4 mb-5">Scan to leave a review for {campaign.businessName}</p>
              ...
              <div className="space-y-3">
                {reviews.map((r) => (
                  <div key={r._id} className="card p-5">
                    ...
                    {r.generatedReview && (
                      <p className="text-sm text-gray-700 mb-3 italic">"{r.generatedReview}"</p>
                    )}
